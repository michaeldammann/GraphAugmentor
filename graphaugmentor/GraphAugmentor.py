...
class GraphTransform_All:
    ...
    def permute_edges(self, data, aug_ratio):
        ...
        for i in range(permute_num):
            idx_add.append(np.random.choice(node_num, 2, replace=False)) #replace=False to avoid self-loops
        idx_add = np.array(idx_add).T
    ...
    def permute_edges_undirected(self, data, aug_ratio):
        ...
        idx_del = np.random.choice(edge_num, (edge_num - permute_num), replace=False)
        print(idx_del)
        edge_index = np.concatenate(
            (edge_index[:, idx_del], idx_add), axis=1)
    ...
    def generate_augmentations_i_j_fast(self, dataset):
        ...
        all_data_i = [self.apply_aug(data_aug_tuple[0], data_aug_tuple[1]) for data_aug_tuple in
                      zip(all_data_i, ri_list)]
        all_data_j = [self.apply_aug(data_aug_tuple[0], data_aug_tuple[1]) for data_aug_tuple in
                      zip(all_data_j, rj_list)]
